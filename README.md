...
**M6 (geometrii ANCPI)** are pipeline-ul de achiziție implementat: sursă ArcGIS FeatureServer înregistrată cu licență CC BY 4.0 (aceeași cu SIRUTA), matching determinist cod SIRUTA → geometrie cu fallback pe nume scopat pe județ (eșuează închis la ambiguitate, nu ghicește), scriere versionată în `registry.territory_geometries` prin `npm run acquire:geometries`. Contractul public v1 e extins aditiv la `contractVersion` 1.1.0 pentru a include, opțional (`required: false`), `territory-geometries.geojson` — un release fără geometrii disponibile publică normal, fără el. Sursa ANCPI rămâne inaccesibilă din mediul interactiv de dezvoltare, dar accesul din runner-ele GitHub Actions a fost verificat direct (HTTP 200); rularea reală (dry-run + publish + un release cu geometrii) încă nu a fost executată. Schema internă rămâne neexpusă public. Harta interactivă (județe pe prima pagină, UAT-uri pe pagina fiecărui județ) e construită direct pe acest contract — SVG generat din geometrii, fără Leaflet/Mapbox/tile server — dar afișează în prezent starea de fallback ("contururile nu sunt încă disponibile"), pentru că release-ul public curent precede rularea reală a pipeline-ului de achiziție; se activează automat la primul release cu geometrii, fără altă modificare de cod.
