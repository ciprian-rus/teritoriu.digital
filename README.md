...
**M11 (hardening)** e parțial: Actions pinned pe SHA, `dependabot.yml`, SBOM în CI, gate `npm audit`, threat model documentat, SLO intern pentru actualizare/disponibilitate, runbook pentru chei/secrete compromise și `GET /api/health` (probă de uptime pe calea reală de citire, fără Supabase); branch protection, secret scanning și execuția reală a exercițiilor de rotație/restore rămân deschise (primele două la deținătorul repo-ului).
