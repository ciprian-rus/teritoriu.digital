...
| M11 — Hardening | observabilitate, backup/restore, incidente și SLA | în lucru — Actions pinned pe commit SHA, `dependabot.yml`, SBOM (CycloneDX) în CI, gate `npm audit --audit-level=high`, threat model documentat (#51, #59), SLO intern (`docs/governance/slo.md`, nu SLA juridic), runbook chei/secrete compromise (`docs/runbooks/incident-compromised-keys.md`) și `GET /api/health` ca probă de uptime; rămân deschise: execuția reală a exercițiilor de rotație/restore, alertare automată pe `/api/health`, cost alerts, iar branch protection și secret scanning depind de acces admin din GitHub Settings al deținătorului repo-ului |
