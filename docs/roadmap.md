...
| M11 — Hardening | observabilitate, backup/restore, incidente și SLA | în lucru — Actions pinned pe commit SHA, `dependabot.yml`, SBOM (CycloneDX) în CI, gate `npm audit --audit-level=high`, threat model documentat (#51, #59), SLO intern (`docs/governance/slo.md`, nu SLA juridic), runbook chei/secrete compromise (`docs/runbooks/incident-compromised-keys.md`) și un drill real de restore (`production-restore-drill.yml`, `pg_dump` read-only din producție restaurat și verificat trimestrial într-o instanță locală efemeră, niciodată scris înapoi); rămân deschise: execuția reală a primului exercițiu de rotație de chei, observabilitate/alertare, cost alerts, iar branch protection și secret scanning depind de acces admin din GitHub Settings al deținătorului repo-ului |
