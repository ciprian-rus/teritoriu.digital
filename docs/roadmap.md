...
| M6 — Geometrii | ANCPI, validare PostGIS, GeoJSON/TopoJSON și hărți | finalizat — pipeline de achiziție rulat real (`scripts/acquire-geometries.mjs`, 3186/3186 UAT-uri matched, zero conflicte), geometrii de județ/București derivate prin uniunea UAT-urilor copil (`scripts/derive-county-geometries.mjs`, 42/42, `geometry_kind: derived`), ambele scrise în `registry.territory_geometries` cu proveniență completă. Contractul public v1 extins aditiv (`contractVersion` 1.1.0) cu `territory-geometries.geojson`, opțional. Release-ul public `2026.08.05.2` include ambele niveluri de geometrie; harta interactivă (județe pe prima pagină, UAT-uri pe pagina fiecărui județ) arată contururi reale, verificată vizual (Playwright, light/dark), axe-core (0 violări) și tastatură |
