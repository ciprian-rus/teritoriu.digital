...
| M6 — Geometrii | ANCPI, validare PostGIS, GeoJSON/TopoJSON și hărți | în lucru — pipeline de achiziție implementat (`scripts/acquire-geometries.mjs`): sursa ANCPI RELUAT rămâne inaccesibilă din sandbox-ul interactiv de dezvoltare, dar accesul din GitHub Actions a fost verificat direct (HTTP 200); matching SIRUTA→geometrie și scrierea în `registry.territory_geometries` sunt testate unitar, rularea reală (dry-run + publish) încă neexecutată |
