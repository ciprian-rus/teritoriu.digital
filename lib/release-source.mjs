...
// 'source_corrected' (a technical ST_MakeValid fix over an invalid
// ANCPI-published 'source' polygon — never a replacement of the original,
// both remain in the bundle) must win over 'source' for the same
// territoryId, since a consumer here needs exactly one geometry per
// territory to render. This priority table makes that explicit instead of
// relying on the artifact's alphabetical sort order (where "source" <
// "source_corrected" happens to already put the correction last, but
// that's an accident of the two names, not a rule).
const GEOMETRY_KIND_PRIORITY = { source_corrected: 1 };

/**
 * Resolves one geometry per territoryId out of a territory-geometries.geojson
 * FeatureCollection that may legitimately contain more than one feature for
 * the same territory (e.g. both the original 'source' and its
 * 'source_corrected' fix). Ties (equal or unknown priority) keep the last
 * feature encountered, matching the artifact's own deterministic
 * [territoryId, geometryKind, detailLevel] sort order.
 */
export function resolveGeometriesByTerritoryId(featureCollection) {
  const map = new Map();
  for (const feature of featureCollection.features) {
    const { territoryId, geometryKind } = feature.properties;
    const existing = map.get(territoryId);
    const priority = GEOMETRY_KIND_PRIORITY[geometryKind] ?? 0;
    if (!existing || priority >= existing.priority) {
      map.set(territoryId, { geometry: feature.geometry, priority });
    }
  }
  return new Map([...map].map(([territoryId, value]) => [territoryId, value.geometry]));
}
...
export async function loadVerifiedRelease() {
  ...
  inFlight = (async () => {
    ...
    const geometriesByTerritoryId = artifacts.has("territory-geometries.geojson")
      ? resolveGeometriesByTerritoryId(
          JSON.parse(artifacts.get("territory-geometries.geojson").toString("utf8"))
        )
      : null;
    cachedRelease = {
